...
fn solve2(data: &str) -> u64 {
    ...
    let groups: Vec<Vec<Rucksack>> = lines
        .chunks(3)
        .map(|c| c.iter().map(|s| Rucksack::from(s)).collect())
        .collect();
    let badges = groups.iter().filter_map(|g| {
        g[0].clone()
            .find_common_item_from_three(g[1].clone(), g[2].clone())
    });
